...
class App extends PureComponent {
    ...
    onRenderedImage = (index) => {
        ...
        if (this.referenceLines.crossViewer !== undefined && this.state.visibleReferenceLines && this.isSliceChange && this.referenceLines.crossViewer.layoutIndex === index) {
    ...
    handleOpenImage = (index) => {
        ...
        if (visibleMprOrthogonal) {
          ...
          this.dicomViewersRefs[2].mprRenderYZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData)
            ...
            } else if ((plane === "sagittal" && visibleMprSagittal) || (plane === "axial" && visibleMprAxial) || (plane === "coronal" && visibleMprCoronal))
                this.dicomViewersRefs[this.props.activeDcmIndex].runTool("openimage", index);
            else if (plane === "sagittal" && visibleMprAxial) this.dicomViewersRefs[this.props.activeDcmIndex].mprRenderXZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData);
            else if (plane === "sagittal" && visibleMprCoronal) this.dicomViewersRefs[this.props.activeDcmIndex].mprRenderYZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData);
            else if (plane === "axial" && visibleMprSagittal) this.dicomViewersRefs[this.props.activeDcmIndex].mprRenderYZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData);
            else if (plane === "axial" && visibleMprCoronal) this.dicomViewersRefs[this.props.activeDcmIndex].mprRenderXZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData);
            else if (plane === "coronal" && visibleMprSagittal) this.dicomViewersRefs[this.props.activeDcmIndex].mprRenderYZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData);
            else if (plane === "coronal" && visibleMprAxial) this.dicomViewersRefs[this.props.activeDcmIndex].mprRenderXZPlane(this.dicomViewersRefs[0].filename, plane, index, this.mprData);
            else {
    ...
    openMultipleFilesCompleted = () => {
        if (this.props.files !== null) {
            ...
            if (sliceMax > 1) this.setState({ visibleExplorer: true, visibleFileManager: false });
        }
    };
    ...
    buildLayoutGrid = () => {
        ...
        for (let i = 0; i < this.props.layout[0]; i++) {
            for (let j = 0; j < this.props.layout[1]; j++) {
                ...
                const index = i * 4 + j;
                dicomviewers.push(
                    <div key={index} style={styleLayoutGrid} onClick={() => this.layoutGridClick(index)} onTouchStart={() => this.layoutGridTouch(index)}>
...
*/
            ...
            if (interpolationMethod === "no") {
                ...
                for (let i = 0; i < intervals.length - 1; i++) for (let j = intervals[i] + 1; j <= intervals[i + 1] - 1; j++) this.volume[j] = this.volume[intervals[i + 1]];
    ...
    changeToSagittalView = () => {
        ...
        this.setState({ visibleVolumeBuilding: false }, () => {
            ...
            if (this.dicomViewersRefs[this.props.activeDcmIndex].volume === null) this.dicomViewersRefs[this.props.activeDcmIndex].volume = this.volume;
    ...
    mprPlanePosition = (force = false, index = this.props.activeDcmIndex) => {
        ...
        if (this.mprPlane === "" || force) {
            ...
            if (!this.state.visibleMprOrthogonal) {
                if (this.mprPlane === "sagittal") this.setState({ visibleMprOrthogonal: false, visibleMprSagittal: true, visibleMprAxial: false, visibleMprCoronal: false });
                else if (this.mprPlane === "coronal") this.setState({ visibleMprOrthogonal: false, visibleMprSagittal: false, visibleMprAxial: false, visibleMprCoronal: true });
                else this.setState({ visibleMprOrthogonal: false, visibleMprSagittal: false, visibleMprAxial: true, visibleMprCoronal: false });
            }
        }
    ...
    getActiveDcmViewersSamePlane = () => {
        ...
        for (let i = 0; i < this.dicomViewersActiveSameStudy.length; i++) {
            ...
            if (this.dicomViewersActiveSameStudy[i].mprPlane === plane && this.dicomViewersActiveSameStudy[i].layoutIndex !== this.props.activeDcmIndex)
    ...
    syncActiveDcmViewersSamePlane = (direction) => {
        ...
        if (this.dicomViewersActiveSamePlane.length > 0) {
            ...
            for (let i = 0; i < this.dicomViewersActiveSamePlane.length; i++) {
                if (plane === "sagittal") j = this.dicomViewersActiveSamePlane[i].findFirstSliceWithIppValue(ippX, 0);
                else if (plane === "coronal") j = this.dicomViewersActiveSamePlane[i].findFirstSliceWithIppValue(ippY, 1);
                else if (plane === "axial") j = this.dicomViewersActiveSamePlane[i].findFirstSliceWithIppValue(ippZ, 2);
    ...
    render() {
        ...
        if (this.dicomViewersRefs[this.props.activeDcmIndex] === undefined) {
            ...
            if (this.dicomViewersRefs[this.props.activeDcmIndex].files !== null) this.isMultipleFiles = this.dicomViewersRefs[this.props.activeDcmIndex].files.length > 1;
