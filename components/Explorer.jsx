...
import { seriesStore, filesStore, explorer, explorerActivePatientIndex, explorerActiveStudyIndex, explorerActiveSeriesIndex } from "../actions";
    ...
    buildPreviewStack = (rows) => {
        ...
        for (let i = 0; i < rows; i++) {
            this.dicomviewers.push(
                <div key={i} onClick={() => this.previewStackClick(i)} onTouchStart={() => this.previewStackTouch(i)}>
                    {this.getDcmViewer(i)}
                </div>
            );
        }
                ...
                <div style={styleScrollbar}>
                    <div style={style}>
                        <FormControl className={classes.formControl}>
                            ...
                            <Select className={classes.selectText} labelId="patient-select-label" id="patient-select" value={this.state.patientName} onChange={this.handlePatientChange}>
                                {this.props.explorer.patient.keys.map((patient, index) => (
                                    <MenuItem className={classes.menuItemText} value={patient} key={index}>
                                        {patient}
                                    </MenuItem>
                                ))}
                            </Select>
                        ...
                        <FormControl className={classes.formControl}>
                            ...
                            <Select className={classes.selectText} labelId="study-select-label" id="study-select" value={this.state.study} onChange={this.handleStudyChange}>
                                {this.state.studies.map((study, index) => (
                                    // `${studyDate} - ${studyTime}`
                                    <MenuItem className={classes.menuItemText} value={study} key={index}>
                                        {study}
                                    </MenuItem>
                                ))}
                            </Select>
                        ...
                        <div style={styleDicomViewerStack}>{this.buildPreviewStack(this.state.series.length)}</div>
                    </div>
                </div>
            </PerfectScrollbar>
        );
