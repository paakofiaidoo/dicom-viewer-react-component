...
class OpenMultipleFilesDlg extends PureComponent {
    ...
    componentDidMount() {
        ...
        for (let i = 0; i < files.length; i++) {
            const file = files[i];
            if (this.props.origin === "local") imageIds.push(cornerstoneWADOImageLoader.wadouri.fileManager.add(file));
            // it's fs item
            else imageIds.push(cornerstoneWADOImageLoader.wadouri.fileManager.addBuffer(file.data));
        }
