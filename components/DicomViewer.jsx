...
import { clearStore, dcmIsOpen, activeDcm, dcmTool, activeMeasurements, doFsRefresh, setDcmEnableTool } from "../actions";
...
class DicomViewer extends React.Component {
    ...
    onImageRendered = (e) => {
        ...
        document.getElementById(`mrbottomright-${this.props.index}`).textContent = `Zoom: ${this.zoom}%`;
    ...
    mprBuildYZPlane = (x) => {
        ...
        for (var y = 0; y < this.ySize; y++) for (var z = 0; z < this.zSize; z++) plane[y + this.ySize * z] = this.volume[z][x + this.ySize * y];
        return plane;
    };
    ...
    mprBuildXZPlane = (y) => {
        ...
        for (let x = 0; x < this.xSize; x++) for (let z = 0; z < this.zSize; z++) plane[x + this.xSize * z] = this.volume[z][x + this.xSize * y];
        return plane;
    };
    ...
    referenceLinesBuild = (srcImage) => {
        ...
        this.referenceLines.isReferenceLine =
            this.referenceLines.dst.orientation !== undefined && this.referenceLines.src.orientation !== undefined && this.referenceLines.dst.orientation !== this.referenceLines.src.orientation;
    ...
    referenceLinesBuildLine = () => {
        ...
        if (!areEqual(nB, nA)) {
            const t = (nP - nA) / (nB - nA);
            if (t > 0 && t <= 1) list.push(src.topLeft.add(src.topRight.sub(src.topLeft).mul(t)));
        }

        if (!areEqual(nC, nB)) {
            const t = (nP - nB) / (nC - nB);
            if (t > 0 && t <= 1) list.push(src.topRight.add(src.bottomRight.sub(src.topRight).mul(t)));
        }

        if (!areEqual(nD, nC)) {
            const t = (nP - nC) / (nD - nC);
            if (t > 0 && t <= 1) list.push(src.bottomRight.add(src.bottomLeft.sub(src.bottomRight).mul(t)));
        }

        if (!areEqual(nA, nD)) {
            const t = (nP - nD) / (nA - nD);
            if (t > 0 && t <= 1) list.push(src.bottomLeft.add(src.topLeft.sub(src.bottomLeft).mul(t)));
        }
    ...
    referenceLinesBuildPlane = () => {
        ...
        let rotation = new Matrix(dst.rowDir.toArray(), dst.colDir.toArray(), dst.nrmDir.toArray());
    ...
    render() {
        ...
        return (
            <div className="container" style={styleContainer}>
                ...
                <Dialog open={errorOnOpenImage !== null} onClose={this.onErrorOpenImageClose}>
                    <DialogTitle id="alert-dialog-title">{"Error on opening image"}</DialogTitle>
                    <DialogContent>
                        <DialogContentText id="alert-dialog-description">{this.state.errorOnOpenImage}</DialogContentText>
                    </DialogContent>
                    <DialogActions>
                ...
                <Dialog open={this.state.errorOnCors} onClose={this.onErrorCorsClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                    <DialogTitle id="alert-dialog-title">{"Error on loading image"}</DialogTitle>
                    <DialogContent>
                        <DialogContentText id="alert-dialog-description">
                            <Typography gutterBottom>
                                CORS or Cross Origin Resource Sharing is a browser security policy that prevents javascript from loading data from a server with a different base URL than the server
                                that served up the javascript file.
                            </Typography>
                            <Typography gutterBottom>
                                See the &nbsp;
                ...
                >
                    <div id={`viewer-${this.props.index}`} ref={this.dicomImageRef} style={styleDicomImage}></div>

                    <div id={`mrtopleft-${this.props.index}`} style={{ position: "absolute", top: 0, left: 3, display: isOpen && overlay ? "" : "none" }}></div>
                    <div id={`mrtopright-${this.props.index}`} style={{ position: "absolute", top: 0, right: 3, display: isOpen && overlay ? "" : "none" }}></div>
                    <div id={`mrbottomright-${this.props.index}`} style={{ position: "absolute", bottom: 0, right: 3, display: isOpen && overlay ? "" : "none" }}></div>
                    <div id={`mrbottomleft-${this.props.index}`} style={{ position: "absolute", bottom: 0, left: 3, display: isOpen && overlay ? "" : "none" }}></div>

                    <div id={`mrtopcenter-${this.props.index}`} style={{ position: "absolute", top: 0, width: "60px", left: "50%", marginLeft: "0px", display: isOpen && overlay ? "" : "none" }}></div>
                    ...
                    {this.state.visibleCinePlayer && this.numberOfFrames > 1 ? (
                        <div style={{ position: "absolute", width: "100%", bottom: 0, textAlign: "center" }}>
                            <div style={{ margin: "0 auto", width: "240px", backgroundColor: "rgba(136, 136, 136, 0.5)" }}>
                                ...
                                <div id={`frameLabel-${this.props.index}`} style={{ width: 230, margin: "0 auto", marginTop: -10, textAlign: "center" }}>
